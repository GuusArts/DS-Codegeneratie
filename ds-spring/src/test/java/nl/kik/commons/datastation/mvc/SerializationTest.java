...
//        Assertions.assertEquals(request, result.getResponse().getContentAsString());
	...
	void request2() throws Exception {
		MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/didcomm/request2")) //
//				.andDo(print()) //
				.andExpect(MockMvcResultMatchers.status().isOk()) //
				.andReturn();
		final String request = result.getResponse().getContentAsString();
...
//        Assertions.assertEquals(request, result.getResponse().getContentAsString());
	...
	void request3() throws Exception {
		MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/didcomm/request3")) //
//				.andDo(print()) //
				.andExpect(MockMvcResultMatchers.status().isOk()) //
				.andReturn();
		final String request = result.getResponse().getContentAsString();
