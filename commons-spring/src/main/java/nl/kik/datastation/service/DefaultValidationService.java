...
public class DefaultValidationService implements ValidationService {
	...
	public void validate(JWSObject jws, Token t, HttpInputMessage headers) throws Exception {
		...
		if (t instanceof Message<?>) {
			validate(jws, (Message<?>) t, claims, headers);
		} else if (t instanceof VerifiableBase) {
			validate(jws, (VerifiableBase) t, claims, headers);
		} else {
			validateExtension(jws, t, claims, headers);
		}
	}

	protected void validateExtension(JWSObject jws, Token t, JWTClaimsSet claims, HttpInputMessage headers)
			throws Exception {
		throw new ParseException("Received object of unexpected type " + t.getClass().getCanonicalName(), 0);
	}

	protected <T> void validate(JWSObject jws, Message<T> t, JWTClaimsSet claims, HttpInputMessage headers)
			throws Exception {
		validateSignature(
	...
	protected void validate(JWSObject jws, VerifiableBase t, JWTClaimsSet claims, HttpInputMessage headers)
			throws Exception {
		validateSignature(
				keys.getVerifier(jws.getHeader().getAlgorithm(), claims.getIssuer(), jws.getHeader().getKeyID()), jws,
				claims);
